  ...
  return (
    <div className="flex items-center justify-center">
      <label className="relative flex items-center justify-center w-8 h-8 cursor-pointer">
        ...
        <span
          className={`absolute w-8 h-8 rounded-full transition-all duration-300 bg-white 
          ${checked
        ...
        {/* Icon / Label */}
        <span
          className={`relative z-10 text-3xl font-bold transition-all duration-300 
          ${checked ? "text-gray-800" : "text-gray-900"}`}
        >
          <StarIcon className={`w-5  ${checked ? "text-yellow-500" : "text-slate-400"}`} />
        </span>
      </label>
    </div>
  );
